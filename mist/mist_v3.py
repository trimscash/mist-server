...
class target_model(nn.Module):
    ...
    def get_components(self, x, no_loss=False):
        ...
        z = self.model.get_first_stage_encoding(
            self.model.encode_first_stage(x)).to(device)
...
def infer(img: PIL.Image.Image, config, tar_img: PIL.Image.Image = None, mask: PIL.Image.Image = None) -> np.ndarray:
    ...
    attack = LinfPGDAttack(net, fn, epsilon, steps,
                           eps_iter=alpha, clip_min=-1.0, targeted=True)
    ...
    if args.input_dir_path:
        ...
        for img_id in os.listdir(image_dir_path):
            ...
            class_name = '_' + str(epsilon) + '_' + str(steps) + '_' + str(input_size) + '_' + str(
                block_num) + '_' + str(mode) + '_' + str(args.rate) + '_' + str(int(mask)) + '_' + str(int(resize))
        ...
        if mask:
            ...
            processed_mask = load_image_from_path(
                mask_path, target_size[0], target_size[1])
        else:
            processed_mask = None
        for i in tqdm(range(block_num)):
            for j in tqdm(range(block_num)):
